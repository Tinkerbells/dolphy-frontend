...
import { IsBoolean, IsEnum, IsNotEmpty, IsOptional, IsString, validateSync } from 'class-validator'

/**
 * Поддерживаемые режимы окружения для Vite
 */
...
export class ViteEnvironmentVariables {
  ...
  // Добавьте больше переменных окружения по мере необходимости для вашего проекта
}

/**
 * Сервис для загрузки и валидации переменных окружения Vite
 */
export class ViteEnvironmentService {
...
/**
 * Создать синглтон-экземпляр сервиса окружения
 */
export const env = new ViteEnvironmentService()
